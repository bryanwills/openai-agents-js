...
>(state: TState): TState {
  ...
  Object.defineProperty(state, 'manifest', {
    ...
    set: (nextManifest: Manifest) => {
      ...
      if (!validReplacement) {
        throw new UserError(
          'A live sandbox manifest cannot remove or replace protected ProcessEnvValue bindings. Create a fresh sandbox session instead.',
        );
      }
...
export function cloneManifestWithProcessEnvironmentAccess(
  ...
  if (invalidReferences.length > 0) {
    throw new UserError(
      `A bound sandbox manifest contains changed ProcessEnvValue references: ${invalidReferences.join(', ')}. Create a fresh sandbox session instead.`,
    );
  }
...
export function assertProcessEnvValuesUnsupported(
  ...
  throw new SandboxUnsupportedFeatureError(
    `${backendId} does not support ProcessEnvValue because it cannot transport protected values through an isolated provider-native creation environment. Use DockerSandboxClient or DaytonaSandboxClient instead.`,
  );
}
...
export function assertProcessEnvironmentAccessBound(manifest: Manifest): void {
  for (const [destination, value] of Object.entries(manifest.environment)) {
    ...
    if (processEnvironmentSources.get(value) !== sourceName) {
      throw new SandboxUnsupportedFeatureError(
        'The configured sandbox client cannot prepare ProcessEnvValue references through trusted runtime options. Use DockerSandboxClient or DaytonaSandboxClient instead.',
      );
    }
  }
}
